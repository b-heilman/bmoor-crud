...
function secure(crud, settings) {
	...
	if (settings.readPermission) {
		accessCfg.beforeRead = function (params, ctx, service) {
			if (
				!(
					ctx.hasPermission(settings.readPermission) ||
	...
	if (settings.createPermission) {
		accessCfg.beforeCreate = async function (datum, ctx, service) {
			if (
				!(
					ctx.hasPermission(settings.createPermission) ||
					(adminPermission && ctx.hasPermission(adminPermission))
				)
			) {
	...
	if (settings.updatePermission) {
		accessCfg.beforeUpdate = async function (datum, ctx, service, delta) {
			if (
				!(
					ctx.hasPermission(settings.updatePermission) ||
					(adminPermission && ctx.hasPermission(adminPermission))
				)
			) {
	...
	if (settings.deletePermission) {
		accessCfg.beforeDelete = async function (key, datum, ctx, service) {
			if (
				!(
					ctx.hasPermission(settings.deletePermission) ||
					(adminPermission && ctx.hasPermission(adminPermission))
				)
			) {
	...
	if (settings.canCreate) {
		crud.security.canCreate = boolWrap(async function (datum, ctx) {
			return (
				(await settings.canCreate(datum, ctx)) ||
				!!(adminPermission && ctx.hasPermission(adminPermission))
			);
	...
	if (settings.canRead) {
		crud.security.canRead = boolWrap(async function (datum, ctx) {
			return (
				(await settings.canRead(datum, ctx)) ||
				!!(adminPermission && ctx.hasPermission(adminPermission))
			);
	...
	if (settings.canUpdate) {
		crud.security.canUpdate = boolWrap(async function (datum, ctx) {
			return (
				(await settings.canUpdate(datum, ctx)) ||
				!!(adminPermission && ctx.hasPermission(adminPermission))
			);
	...
	if (settings.canDelete) {
		crud.security.canDelete = boolWrap(async function (datum, ctx) {
			return (
				(await settings.canDelete(datum, ctx)) ||
				!!(adminPermission && ctx.hasPermission(adminPermission))
			);
	...
	if (settings.filterPermission) {
		const filterFn = function (ctx) {
			return () =>
				ctx.hasPermission(settings.filterPermission) ||
				!!(adminPermission && ctx.hasPermission(adminPermission));
		};
	...
	if (settings.filterFactory) {
		crud.security.filterFactory = filterFactory(function (ctx) {
			const fn = settings.filterFactory(ctx);

			return (datum) =>
				fn(datum) || !!(adminPermission && ctx.hasPermission(adminPermission));
		}, crud.security.filterFactory);
	}
}
