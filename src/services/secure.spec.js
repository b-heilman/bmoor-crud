	...
	describe('base functionality', function () {
		...
		describe('::canCreate', function () {
			it('should define a base method', async function () {
				...
				expect(await base.security.canCreate({id: 10})).to.equal(true);

				expect(await base.security.canCreate({id: 20})).to.equal(false);
			});
			...
			it('should define with multiple', async function () {
				...
				expect(await base.security.canCreate({id: 3})).to.equal(false);

				expect(await base.security.canCreate({id: 2})).to.equal(false);

				expect(await base.security.canCreate({id: 6})).to.equal(true);
			});
		});
		...
		describe('::canRead', function () {
			it('should define a base method', async function () {
				...
				expect(await base.security.canRead({id: 10})).to.equal(true);

				expect(await base.security.canRead({id: 20})).to.equal(false);
			});
			...
			it('should define with multiple', async function () {
				...
				expect(await base.security.canRead({id: 3})).to.equal(false);

				expect(await base.security.canRead({id: 2})).to.equal(false);

				expect(await base.security.canRead({id: 6})).to.equal(true);
			});
		});
	...
	describe('via a Service', function () {
		...
		describe('::canCreate', function () {
			describe('.create', function () {
				beforeEach(function () {
					stubs.execute.resolves([{eins: 10, zwei: 20, drei: 30}]);
				});
		...
		describe('::canRead', function () {
			describe('.read', function () {
				...
				it('should fail', async function () {
					...
					stubs.execute.resolves([{eins: 10, zwei: 21, drei: 30}]);
					...
					try {
						...
						expect(ex.code).to.equal('BMOOR_CRUD_SERVICE_CAN_READ');
					}
