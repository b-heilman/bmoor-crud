...
class Document extends View {
	async link() {
		...
		await Promise.all(
			this.structure.subs.map(async (sub) => {
				sub.document = await sub.composite.nexus.loadDocument(
					sub.composite.name
				);
			})
		);
			...
			res.map(async (queryDatum) => {
				...
				await Promise.all(
					this.structure.subs.map(async ({info, mounts, document, params}) => {
						...
						set(queryDatum, info.path, info.isArray ? res : res[0]);
					})
				);
	...
	async normalize(incomingDatum, parentSession, ctx) {
		...
		const cbs = await Promise.all(
			Object.keys(transitions).map(async (series) => {
				...
				return () => {
					...
					links.forEach((link) => {
						...
						newDatum.setField(link.local, target.getReference().getHash());
					});
				};
			})
		);
		...
		await Promise.all(
			this.structure.subs.map((sub) => {
				...
				return Promise.all(
					content.map(async (subDatum) => {
						const {seriesSession: subSession, changeType: subChange} =
							await sub.document.normalize(subDatum, seriesSession, ctx);

						changeType = compareChanges(changeType, subChange);

						const mountPath = this.structure.instructions.getMount(
							sub.composite.name
						);
		...
		if (this.structure.incomingSettings.onChange && changeType) {
			await this.structure.incomingSettings.onChange(
				changeType,
				seriesSession,
				ctx
			);
	...
	async getAffectedByModel(modelName, key, ctx) {
		const exe = new Executor(
			...
			await this.structure.getKeyQueryByModel(modelName, key, ctx)
		);

		return (await this.run(exe, ctx)).map((response) => response.key);
	}

	async getAffectedBySub(subName, key, ctx) {
		const exe = new Executor(
			this.structure.name + ':byModel',
			await this.structure.getKeyQueryBySeries(subName, key, ctx)
		);
