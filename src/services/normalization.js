...
async function ensure(mapper, modelName, payload, ctx) {
	return Promise.all(
		mapper.getByDirection(modelName, 'outgoing').map(async (link) => {
...
async function install(datum, service, master, mapper, ctx) {
	...
	if (datum.getAction() === 'read') {
		...
		if (!rtn) {
			throw create(
				`unable to read expected datum of type ${service.structure.name}`,
				{
					status: 406,
		...
		if (datum.getAction() === 'update') {
			...
			action = 'update';
			rtn = await service.update(
				service.structure.getKey(current),
				content,
				ctx
			);
...
async function deflate(schema, nexus, ctx) {
	...
	return order.reduce(async (prom, serviceName) => {
		const service = await nexus.loadCrud(serviceName);

		return master
			.get(serviceName)
			.toArray()
			.reduce(async (prom, datum) => {
				const agg = await prom;

				const res = await install(datum, service, master, nexus.mapper, ctx);
...
async function inflate(service, query, nexus, ctx) {
	...
	const stubModels = (query.stub || []).reduce(
		// read
		(agg, table) => {
			agg[table] = true;
		...
		await Promise.all(
			datums.map(async (datum) => {
				...
				if (!stubbed) {
					...
					await Promise.all(
						nexus.mapper
							.getByDirection(service.structure.name, 'outgoing')
							.map(async (link) => {
								// jshint ignore:line
					...
					if (lookup) {
						nexus.mapper
							.getByDirection(service.structure.name, 'incoming')
							.forEach((link) => {
								if (lookup[link.name]) {
									toProcess.push({
										ref: ref,
