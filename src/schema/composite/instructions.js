...
function absorbIndexMerge(target, source, namespace=''){
	Object.keys(source).forEach(series => {
		...
		const incoming = additional.incoming.map(inSeries => namespace+inSeries);
		...
		if (additional.composite){
			...
			const namespacedJoin = Object.keys(additional.join)
			.reduce(
				(agg, joinedSeries) => {
...
class Instructions {
	...
	addStatement(mount, statement, namespace=''){
		...
		if  (!join){
			throw create(`requesting field not joined ${namespace?namespace+':':''}${action.series}`, {
	...
	inline(series, instructions){
		...
		this.subs = this.subs.filter(info => info.path !== path)
		.concat(
			instructions.subs.map(
				info => {
