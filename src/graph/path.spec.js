	...
	describe('Path', function(){
		describe('.access', function(){
			...
			it('should properly parse a multipart string with optional and carry it', function(){
				expect((new sut.Path('$foo-bar.field > ?$hello-world.id > $eins-zwei')).access)
				.to.deep.equal([{
					loader: 'access',
					model: 'foo-bar',
					series: 'foo-bar',
				...
				expect((new sut.Path('$test-5.ownerId > ?$owner:test-1.title')).access)
				.to.deep.equal([{
					loader: 'access',
					model: 'test-5',
					series: 'test-5',
			...
			it('should properly parse method', function(){
				expect((new sut.Path('=helloWorld($foo-bar.eins, $hello-world.zwei)')).access)
				.to.deep.equal([{
					loader: 'method',
					arguments: [
						[{
		...
		describe('.path', function(){
			...
			it('should properly join into a multipart string', function(){
				...
				.to.equal('$foo-bar.field>$hello-world.id>?$eins-zwei.id');
			});
		});
	});
});
